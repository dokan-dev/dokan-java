 ...
 * important callbacks such as {@link DokanyOperations.CreateFile} or {@link DokanyOperations.ReadFile} would make the file system not working or unstable.
 *
...
public class DokanyOperations extends Structure {
	 ...
	 */
	@FunctionalInterface
	interface CreateFile extends Callback {
		/**
		 * @param rawFileName Path requested by the Kernel on the File System.
		 ...
		 */
		long callback(
		        @NotNull WString rawFileName,
		        @NotNull IntByReference securityContext,
	...
	/**
	 * ReadFile callback on the file previously opened in {@link DokanyOperations.CreateFile}. It can be called by different thread at the same time, therefore the read has to be
	 * thread safe.
	 *
	 */
	...
	/**
	 * WriteFile callback on the file previously opened in {@link DokanyOperations.CreateFile} It can be called by different thread at the same time, therefore the write/context
	...
	interface SetFileTime extends Callback {
		/**
		 *
		 * @param rawFileName path to file or directory
