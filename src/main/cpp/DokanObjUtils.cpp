...
int GetOperationResult(JNIEnv* env)
{
	...
	if (env->IsInstanceOf(t, dokanExceptionClass)) {
		result = -(env->GetIntField(t, errorCodeID));
		LOG(L"[GetOperationResult] dokanException. ErrorCode = %d\n", -result);
	} else {
...
jobject ToDokanFileInfoJavaObject(JNIEnv* env, PDOKAN_FILE_INFO DokanFileInfo) throw(...)
{
	...
	jclass clz = env->FindClass("com/github/dokandev/dokanjava/DokanFileInfo");
	if (clz == NULL) 
		return NULL;
